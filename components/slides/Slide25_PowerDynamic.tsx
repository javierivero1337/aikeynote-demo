...
export function Slide25_PowerDynamic() {
  const [taskIndex, setTaskIndex] = useState(0);

  useEffect(() => {
    const interval = setInterval(() => {
      setTaskIndex((prev) => (prev + 1) % OBSOLETE_TASKS.length);
    }, 2500);
          ...
          <motion.div variants={columnVariants} className="space-y-8">
            ...
            <div className="space-y-6">
              ...
              >
                ...
                <div className="text-zinc-500 text-lg group-hover:text-zinc-400 transition-colors flex flex-wrap items-center gap-x-2">
                  <span>Si tu valor reside únicamente en la</span>
                  <div className="relative h-7 min-w-[180px] inline-flex items-center">
                    <AnimatePresence mode="wait">
                      <motion.span
                        key={OBSOLETE_TASKS[taskIndex]}
                        initial={{ y: 10, opacity: 0, filter: "blur(5px)" }}
                        animate={{ y: 0, opacity: 1, filter: "blur(0px)" }}
                        exit={{ y: -10, opacity: 0, filter: "blur(5px)" }}
                        transition={{ duration: 0.4, ease: "easeInOut" }}
                        className="text-zinc-400 group-hover:text-zinc-300 font-medium decoration-red-500/30 underline decoration-wavy underline-offset-4 absolute whitespace-nowrap"
                      >
                        {OBSOLETE_TASKS[taskIndex]}
                      </motion.span>
                    </AnimatePresence>
                  </div>
                </div>
              ...
              >
                ...
                <p className="text-zinc-300 text-xl relative z-10 leading-relaxed">
                  Si dejas de ser el <span className="text-white font-semibold">ejecutor</span> para convertirte en el <span className="text-green-400 font-bold">arquitecto</span> y <span className="text-green-400 font-bold">juez</span> de lo que la IA produce.
                </p>
              </motion.div>
            </div>
