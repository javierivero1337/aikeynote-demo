...
function CardInner({ tweet, size }: { tweet: Tweet; size: "sm" | "lg" }) {
  ...
  return (
    <div
      className={`relative rounded-2xl border border-zinc-800/60 bg-zinc-950/90 backdrop-blur-sm overflow-hidden ${
        isLg ? "p-8" : "p-5"
      }`}
    >
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-blue-500/[0.02] via-transparent to-purple-500/[0.02] pointer-events-none" />

      <div className="relative z-10">
        {/* Header */}
        <div className={`flex items-center gap-3 ${isLg ? "mb-5" : "mb-3"}`}>
          <div
            className={`rounded-full bg-gradient-to-br from-zinc-700 to-zinc-800 flex items-center justify-center font-bold text-zinc-400 shrink-0 border border-zinc-700/50 ${
              isLg ? "w-12 h-12 text-base" : "w-9 h-9 text-xs"
            }`}
          >
            {tweet.name.charAt(0)}
          </div>
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
        <motion.div
          ...
          onClick={onClose}
        >
          {/* Intensified glow when expanded */}
          <motion.div
            className="absolute -inset-1 rounded-2xl pointer-events-none"
            style={{
              boxShadow: `0 0 40px 8px ${tweet.glowColor}, 0 0 100px 20px ${tweet.glowColor}`,
            }}
...
/* ── Main slide ── */
export function Slide06_Tweets() {
